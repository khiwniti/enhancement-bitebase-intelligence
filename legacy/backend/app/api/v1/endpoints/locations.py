...
from app.utils.validation import (
    validate_coordinates, validate_numeric_range, validate_list_items,
    validate_string_length, ValidationError
)
from app.middleware.error_handling import (
    create_success_response, create_error_response, create_validation_error
)
...
async def analyze_location(
    ...
    try:
        ...
        # Validate radius
        validated_radius = validate_numeric_range(radius_km, "radius_km", 0.1, 10.0)

        # Validate cuisine types if provided
        if cuisine_types:
            validated_cuisine_types = validate_list_items(
                cuisine_types,
                "cuisine_types",
                max_items=10,
                item_validator=lambda x: validate_string_length(x, "cuisine_type", 1, 50)
            )
        else:
            validated_cuisine_types = None

        # Validate category if provided
        validated_category = None
        if category:
            validated_category = validate_string_length(category, "category", 1, 50)
        ...
        analysis_result = await location_service.analyze_location(
            ...
            radius_km=validated_radius,
            target_cuisine_types=validated_cuisine_types,
            target_category=validated_category
        )
        ...
        raise create_validation_error(e.field, e.message, e.value)
    except Exception as e:
        raise create_error_response(
            f"Location analysis failed: {str(e)}",
            status_code=500,
...
async def compare_locations(
    ...
    """
    Compare multiple locations for business potential
    """
    try:
        if len(locations) < 2:
        ...
        for i, location in enumerate(locations):
            try:
                ...
                comparison_results.append({
                    ...
                    "analysis": analysis,
                    "overall_score": analysis['location_score']['overall_score'],
        ...
        comparison_results.sort(key=lambda x: x.get('overall_score', 0), reverse=True)
        for i, result in enumerate(comparison_results):
            result['rank'] = i + 1

        # Generate comparison insights
        best_location = comparison_results[0] if comparison_results else None
        worst_location = comparison_results[-1] if comparison_results else None

        comparison_insights = {
            "best_location": {
                "name": best_location['name'] if best_location else None,
                "score": best_location['overall_score'] if best_location else 0,
                "key_strengths": []
            },
            "worst_location": {
                "name": worst_location['name'] if worst_location else None,
                "score": worst_location['overall_score'] if worst_location else 0,
                "key_weaknesses": []
            },
            "score_range": {
                "highest": best_location['overall_score'] if best_location else 0,
                "lowest": worst_location['overall_score'] if worst_location else 0,
                "average": sum(r.get('overall_score', 0) for r in comparison_results) / len(comparison_results) if comparison_results else 0
            },
            "recommendations": [
                f"Location '{best_location['name']}' shows the highest potential with a score of {best_location['overall_score']}" if best_location else "No valid analysis results",
                "Consider demographic factors and competition density when making final decisions",
                "Validate findings with on-site visits and local market research"
            ]
        }
