...
async def get_insight_metrics(
    period_type: str = Query("daily", description="Period type for metrics"),
    days_back: int = Query(7, description="Number of days to look back"),
    db: AsyncSession = Depends(get_db)
):
    """
    Get insights metrics summary
    """
    try:
        ...
        return {
            "success": True,
            "data": metrics_data,
            "message": "Insights metrics retrieved successfully"
        }

    except Exception as e:
        logger.error(f"Failed to get insights metrics: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to get insights metrics: {str(e)}")
