...
export const generateMarketReport = onCall({
  cors: true,
...
export const getMarketReports = onCall({
  cors: true
}, async (request) => {
  try {
    if (!request.auth) {
...
export const getMarketReport = onCall({
  cors: true
}, async (request) => {
  try {
    if (!request.auth) {
    ...
    if (reportData?.userId !== userId && !reportData?.isPublic) {
...
export const deleteMarketReport = onCall({
  cors: true
}, async (request) => {
  try {
    if (!request.auth) {
...
async function processMarketReport(
  reportId: string, 
  query: string, 
...
async function gatherMarketData(targetLocation: string, coordinates: any, analysisRadius: number) {
  try {
...
async function generateAIAnalysis(query: string, marketData: any) {
  ...
  const avgRating = marketData.restaurants.length > 0 
    ? marketData.restaurants.reduce((sum: number, r: any) => sum + (r.rating || 0), 0) / restaurantCount
    : 0;
