...
exports.getLocationAnalyses = exports.analyzeLocation = void 0;
const https_1 = require("firebase-functions/v2/https");
const logger = __importStar(require("firebase-functions/logger"));
const firestore_1 = require("firebase-admin/firestore");
const db = (0, firestore_1.getFirestore)();
// Analyze location for market intelligence
exports.analyzeLocation = (0, https_1.onCall)({
    cors: true,
    ...
    try {
        ...
        const { latitude, longitude, radius = 1.0, analysisType = "market_opportunity", address, city, state, zipCode } = request.data;
        ...
        return {
            success: true,
            data: Object.assign(Object.assign({ id: analysisRef.id }, analysisData), { rawData: undefined // Don't return raw data in response
             })
        };
    }
    catch (error) {
...
exports.getLocationAnalyses = (0, https_1.onCall)({
    cors: true
}, async (request) => {
    try {
        if (!request.auth) {
        ...
        const snapshot = await db.collection("location_analyses")
            ...
            .offset(offset)
            .get();
        const analyses = snapshot.docs.map(doc => (Object.assign(Object.assign({ id: doc.id }, doc.data()), { rawData: undefined // Don't include raw data in list
         })));
        return {
            success: true,
            data: analyses,
...
async function getNearbyRestaurants(latitude, longitude, radius) {
    try {
...
async function performMarketAnalysis(restaurants, analysisType) {
...
async function getDemographicData(latitude, longitude) {
...
function calculateOpportunityScore(marketAnalysis, demographicData) {
...
function generateInsights(marketAnalysis, demographicData, opportunityScore) {
    const insights = [];
    if (marketAnalysis.saturation < 10) {
...
function generateRecommendations(marketAnalysis, demographicData, opportunityScore) {
